...
//  --------------------------------------------Ordenacao ---------------------------------------------
    ...
    //------------- CountingSort por ano de nascimento ----------
    public static void countingSort(List<Personagem> arr) {
        //Array para contar o numero de ocorrencias de cada elemento
        int[] count = new int[getMaior(arr) + 1];
        Personagem[] ordenado = new Personagem[arr.size()];
        ...
        for (int i = arr.size() - 1; i >= 0; i--) {
            ordenado[count[arr.get(i).getYearOfBirth()] - 1] = arr.get(i);
            count[arr.get(i).getYearOfBirth()]--;
        }

        // Copiando para o array original
        for (int i = 0; i < arr.size(); i++) {
            arr.set(i, ordenado[i]);
        }
    }
      ...
      * @return maior elemento
      ...
      // ------------ Merge Sort por actor name --------------
      public static void mergeSort(List<Personagem> arr) {
        if (arr.size() <= 1) {
            return;
        }

        int meio = arr.size() / 2;
        List<Personagem> esquerda = new ArrayList<>(arr.subList(0, meio));
        List<Personagem> direita = new ArrayList<>(arr.subList(meio, arr.size()));
    ...
    private static void merge(List<Personagem> arr, List<Personagem> esquerda, List<Personagem> direita) {
        int i = 0, j = 0, k = 0;

        while (i < esquerda.size() && j < direita.size()) {
            if (esquerda.get(i).getActorName().compareTo(direita.get(j).getActorName()) <= 0) {
    ...
    // ------ Fim MergeSort --------
    // ------ QuickSort por house -------
    public static void quickSort(ArrayList<Personagem> arr){
        quickSortCall(arr, 0, arr.size() - 1,arr.size());
    }
    private static void quickSortCall(ArrayList<Personagem> arr,int inicio, int fim, int ordenacaoTotal) {
        int i = inicio;
        int j = fim;
        Personagem chave = arr.get((inicio + fim) / 2);
        while(j>=i){
            while(compareHouse(arr.get(i), chave) < 0) i++;
            while(compareHouse(chave,arr.get(j)) < 0) j--;
            ...
            if(inicio < j ) quickSortCall(arr, inicio, j, ordenacaoTotal);
            if(ordenacaoTotal > i ) quickSortCall(arr, i, fim, ordenacaoTotal); 
        }
    }
    ...
    public class Main{
        ...
        public static void main(String[] args) throws ParseException {
            ...
            Ordenacao.quickSort(subList1);
            //  apenas 10 personagens
