...
int totalFila(Fila* fila);
void mostrarFila(Fila* fila);
void mostrarFilaCall(CelulaF* aux);
  

// Funçao para inicializar a Fila
void iniciarFila(Fila* fila){
    fila->primeiro = (CelulaF*)malloc(1 * sizeof(CelulaF));
...
void mostrarFilaCall(CelulaF* aux){
    printf("%d ", aux->valor);
    if ( aux->prox != NULL ) mostrarFilaCall(aux->prox);
}
...
void inserirInicioLista(Lista* lista, Personagem personagem);
void inserirFimLista(Lista* lista, Personagem personagem);
void inserirPosiLista(Lista* lista, Personagem personagem, int posi);
Personagem removerInicioLista(Lista* lista);
Personagem removerFimLista(Lista* lista);
Personagem removerPosiLista(Lista* lista, int resp);
void mostrarLista(Lista* lista);
int totalLista(Lista* lista);

void trocar(CelulaL* a, CelulaL* b);
CelulaL* particionar(CelulaL* baixo, CelulaL* alto);
void quicksort(CelulaL* baixo, CelulaL* alto);
void ordenarListaPorNome(Lista* lista);
...
CelulaL* particionar(CelulaL* baixo, CelulaL* alto) {
    Personagem pivo = alto->personagem;
    CelulaL* i = baixo->ant;

    for (CelulaL* j = baixo; j != alto; j = j->prox) {
        if (compareHouse(j->personagem, pivo) <= 0) {
...
void quicksort(CelulaL* baixo, CelulaL* alto) {
    if (alto != NULL && baixo != alto && baixo != alto->prox) {
        CelulaL* p = particionar(baixo, alto);
...
void ordenarListaPorNome(Lista* lista) {
    if (lista->primeiro->prox != NULL) {
        CelulaL* ultimo = lista->ultimo;
        quicksort(lista->primeiro->prox, ultimo);
    }
}
...
void inserirFimLista(Lista* lista, Personagem personagem){
    ...
    lista->ultimo->personagem = personagem;
...
Personagem removerInicioLista(Lista* lista){
    Personagem resp = lista->primeiro->prox->personagem;
...
Personagem removerPosiLista(Lista* lista, int posi){
    CelulaL* tmp = lista->primeiro;
    for ( int i = 0; i < posi; i++ ){
        tmp = tmp->prox;
    }
...
void mostrarLista(Lista* lista){
    ...
    int loop = 0;
    while ( aux != NULL ){
        printPersonagem(aux->personagem, loop++);
        aux = aux->prox;
    }
}
...
int main(){
    ...
    while(strcmp(id, "FIM")){
        ...
        if ( i != -1 ) {
            inserirFimLista(&using, personagens[i]);
        }
