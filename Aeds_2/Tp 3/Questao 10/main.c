...
int main() {
  ...
  array pilha;
  array pilhaRemovidos;
  pilhaRemovidos.size = 0;
  pilhaRemovidos.topo = NULL;
  pilha.topo = NULL;
  
  inicializandoPilha(&pilha, subListaId);
  processaComandos(&Lista, &pilha,&pilhaRemovidos);
  // printaRemovidos(&pilhaRemovidos);
  printaArray(&pilha);

  free(subListaId);
  
  Personagem* currenteRemovidos = pilhaRemovidos.topo;
  while (currenteRemovidos != NULL) {
    Personagem* next = currenteRemovidos->prox;
    free(currenteRemovidos);
    currenteRemovidos = next;
    if (currenteRemovidos == pilhaRemovidos.topo) break; // Fila circular termina no último elemento
  }


  // Libera os elementos da fila circular
  Personagem* current = pilha.topo;
  while (current != NULL) {
    Personagem* next = current->prox;
    free(current);
    current = next;
    if (current == pilha.topo) break; // Fila circular termina no último elemento
  }
