...
void printaArray(array *Lista) {
    int pos = 0;
    printf(" [ Head ]\n");
    for (int i = Lista->primeiro; i != Lista->ultimo; i = (i + 1) % Lista->size) {
        printf("[%d ## %s ## ", pos, Lista->vet[i].id);
        printf("%s ## ", Lista->vet[i].name);
        printf("{%s} ## ", tratandoAlternateNames(Lista->vet[i].alternate_names));
        printf("%s ## ", Lista->vet[i].house);
        printf("%s ## ", Lista->vet[i].ancestry);
        printf("%s ## ", Lista->vet[i].species);
        printf("%s ## ", Lista->vet[i].patronus);
        printf("%s ## ", charToBoolean(Lista->vet[i].hogwartsStaff) ? "true" : "false");
        printf("%s ## ", charToBoolean(Lista->vet[i].hogwartsStudent) ? "true" : "false");
        printf("%s ## ", Lista->vet[i].actorName);
        printf("%s ## ", charToBoolean(Lista->vet[i].alive) ? "true" : "false");
        printf("%s ## ", Lista->vet[i].DataNascimento);
        printf("%s ## ", Lista->vet[i].anoNascimento);
        printf("%s ## ", Lista->vet[i].eyeColour);
        printf("%s ## ", Lista->vet[i].gender);
        printf("%s ## ", Lista->vet[i].hairColour);
        printf("%s]\n", charToBoolean(Lista->vet[i].wizard) ? "true" : "false");
        pos++;
    }
    printf(" [ Tail ]\n");
}
...
int mediaYearOfBirth(array *fila) {
    int soma = 0;
    int count = 0;
    char temp[100];
    for (int i = fila->primeiro; i != fila->ultimo; i = (i + 1) % fila->size) {
        strcpy(temp, fila->vet[i].anoNascimento);
        soma += atoi(temp);
        count++;
    }
    return soma / count;
...
Personagem removerInicio(array *fila) {
    ...
    Personagem temp = fila->vet[fila->primeiro];
    fila->primeiro = (fila->primeiro + 1) % fila->size;
    return temp;
}

void inserirFim(array *fila, Personagem personagem) {
    if (((fila->ultimo + 1) % fila->size) == fila->primeiro) {
        removerInicio(fila);
    }
    fila->vet[fila->ultimo] = personagem;
    fila->ultimo = (fila->ultimo + 1) % fila->size;
    printf(">> Year Birthday Average: %d\n", mediaYearOfBirth(fila));
}

void processaComandos(array *lista, array *fila) {
    int qtd;
    scanf("%d", &qtd);
...
void inicializandoFila(array* fila,array* subLista){
  for(int i = 0;i < subLista->size;i++){
    inserirFim(fila,  subLista->vet[i]);
  }
}
// ------------------------------------------ Funcao Principal
// ------------------------------
int main() {
  ...
  filaCircular.size = 6;
  filaCircular.primeiro = 0;
  filaCircular.ultimo = 0;

  importarBd(arq, &Lista);
  array *subListaId = subListId(&Lista);

  inicializandoFila(&filaCircular,subListaId);
  processaComandos(&Lista,&filaCircular);
  printaArray(&filaCircular);
