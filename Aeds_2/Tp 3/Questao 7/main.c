...
void importarBd(FILE *arq, array *Lista) {
  ...
  fgets(linha, sizeof(linha),
        arq); // Descartar a primeira linha (cabeÃ§alho)
...
void printaArray(array *Lista) {
  int pos = 0;
  printf(" [ Head ]\n");
  for (Personagem* i = Lista->primeiro; i != NULL; i = i->prox) {
        printf("[%d ## %s ## ", pos, i->id);
        printf("%s ## ", i->name);
        ...
        pos++;
    if (i == Lista->ultimo) break; // Fila circular termina no Ãºltimo elemento
  }
  printf(" [ Tail ]\n");
}
...
int mediaYearOfBirth(array *fila) {
  ...
  int count = 0;
  char temp[100];
  for (Personagem* i = fila->primeiro; i != NULL; i = i->prox) {
        strcpy(temp, i->anoNascimento);
        soma += atoi(temp);
        count++;
  }
  return soma/count;
...
void inserirFim(array *fila, Personagem* personagem) {
  if (fila->size == fila->capacity) {
    Personagem* removido = removerInicio(fila);
    free(removido);
  }
  if (fila->ultimo == NULL) {
    fila->primeiro = fila->ultimo = personagem;
  } else {
    fila->ultimo->prox = personagem;
...
void processaComandos(array *lista, array *fila) {
  int qtd;
  scanf("%d", &qtd);
  getchar(); // Consumir o newline apÃ³s o nÃºmero
...
int main() {
  ...
  array filaCircular;
  filaCircular.size = 0;
  filaCircular.capacity = 5;
  filaCircular.primeiro = NULL;
  filaCircular.ultimo = NULL;
  ...
  // Libera os elementos da fila circular
  Personagem* current = filaCircular.primeiro;
  while (current != NULL) {
    Personagem* next = current->prox;
    free(current);
    current = next;
    if (current == filaCircular.primeiro) break; // Fila circular termina no Ãºltimo elemento
  }
